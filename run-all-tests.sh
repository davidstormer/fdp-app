...
then
  ...
  python manage.py test --parallel 4 -v 2 --settings=fdp.configuration.test.test_local_settings || exit
  python manage.py test --parallel 4 -v 2 --pattern=azure_test* --settings=fdp.configuration.test.test_azure_settings || exit
  python manage.py test --parallel 4 -v 2 --pattern=azure_only_test* --settings=fdp.configuration.test.test_azure_only_settings || exit
fi
